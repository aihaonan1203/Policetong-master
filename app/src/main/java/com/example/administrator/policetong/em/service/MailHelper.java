...
package com.example.administrator.policetong.em.service;
...
public class MailHelper {
    ...
    public List<MailReceiver> getAllMail(String folderName) throws MessagingException {
        ...
        Session session = Session.getInstance(props,null); // 取得pop3协议的邮件服务器
...
//        Store store= MyApplication.session.getStore("pop3");
        ...
        Log.e( "getAllMail: ",mailCount+"" );
        if (mailCount == 0) {
            Log.e( "getAllMail: ",11111+"" );
            folder.close(true);
            store.close();
            return null;
        } else {
