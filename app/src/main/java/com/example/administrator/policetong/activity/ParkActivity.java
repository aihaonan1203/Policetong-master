    ...
    private void getNetData() {
        disposable = Network.getPoliceApi().getPark()
                ...
                .subscribe(new Consumer<BaseBean<List<CarBean>>>() {
                    @Override
                    public void accept(BaseBean<List<CarBean>> listBaseBean) throws Exception {
...
//                showDialog(id);
    ...
    private class MyAdapter extends BaseAdapter {
        ...
        public View getView(int i, View view, ViewGroup viewGroup) {
            if (view == null) {
                view = View.inflate(ParkActivity.this, R.layout.car_park_item, null);
                view.setTag(new ViewHolder(view));
            }
            Set_data_into_layout(i, data.get(i), (ViewHolder) view.getTag());
            return view;
        }

        @SuppressLint("SetTextI18n")
        private void Set_data_into_layout(int id, CarBean bean, ViewHolder holder) {
            holder.pc_item_id.setText(id + 1 + "");
            holder.pc_item_other.setText("备注:"+bean.getRemark());
            holder.tv_in_time.setText("入场时间:"+Utils.stampToDate(bean.getEntranceTime()));
            holder.tv_model.setText("车型:"+bean.getModels());
            holder.tv_carNumber.setText("车牌号:"+bean.getLicence());
